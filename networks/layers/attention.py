...
class GatedPropagation(nn.Module):
    ...
    def forward(self, Q, K, V, U, size_2d):
        ...
        # Multi-head
        Q = Q.view(-1, bs, num_head, self.d_att).permute(1, 2, 0, 3) # [1, 1, 1674, 128]
        K = K.view(-1, bs, num_head, self.d_att).permute(1, 2, 3, 0) # [1, 1, 128, 1674]
        V = V.view(-1, bs, num_head, hidden_dim).permute(1, 2, 0, 3) # [1, 1, 1674, 1024]

        # Multiplication
        QK = multiply_by_ychunks(Q, K, self.qk_chunks) # [1, 1, 1674, 1674]
        if self.use_dis:
            QK = 2 * QK - K.pow(2).sum(dim=-2, keepdim=True)
        ...
        # Dropouts
        attn = self.dropout(attn) # (1, 1, 1674, 1674)

        # Weighted sum
        outputs = multiply_by_xchunks(attn, V,
                                      self.qk_chunks).permute(2, 0, 1, 3) # [1674, 1, 1, 1024]

        # Restore shape
        outputs = outputs.reshape(l, bs, -1) * U # [1674, 1, 1024] * [1674, 1, 1024]

        outputs = self.dw_conv(outputs, size_2d)
        outputs = self.projection(outputs) # [1674, 1, 512]
...
class LocalGatedPropagation(nn.Module):
    ...
    def forward(self, q, k, v, u, size_2d):
        ...
        self.last_size_2d = (h, w)
        return output, local_attn # output [1674, 1, 512] local_attn [1, 1, 225, 1674]
