...
class LongShortTermTransformer(nn.Module):
    ...
    def forward(self,
        ...
        for idx, layer in enumerate(self.layers):
            output, memories, att_map = layer(output,
        ...
        return output, memories, att_maps
...
class DualBranchGPM(nn.Module):
    ...
    def forward(self,
        ...
        output = self.emb_dropout(tgt) # [1674, 1, 256] [h * w, n, c]
        ...
        for idx, layer in enumerate(self.layers):
            output, output_id, memories, att_map = layer(
                output,
                output_id,
        ...
        if self.return_intermediate:
            return intermediate, intermediate_memories, att_maps

        return cat_output, memories, att_maps
...
class LongShortTermTransformerBlock(nn.Module):
    ...
    def forward(self,
        ...
        tgt2, attn_map = self.long_term_attn(curr_Q, global_K, global_V)
...
class GatedPropagationModule(nn.Module):
    def __init__(self,
        ...
        # Long Short-Term Attention
        self.norm1 = _get_norm(d_model)
        self.linear_QV = nn.Linear(d_model, d_att * att_nhead + expand_d_model) # (256, 128 * 1 + 512)
        self.linear_U = nn.Linear(d_model, expand_d_model) # (256, 512)
    ...
    def forward(self,
        ...
        # Long Short-Term Attention
        _tgt = self.norm1(tgt) # _tgt [1674, 1, 256] [h * w, n, c]

        curr_QV = self.linear_QV(_tgt) # curr_QV [1674, 1, 640]
        curr_QV = torch.split(
            curr_QV, [self.d_att * self.att_nhead, self.expand_d_model], dim=2) # curr_QV [[1674, 1, 128], [1674, 1, 512]]
        curr_Q = curr_K = curr_QV[0] # [1674, 1, 128]
        local_Q = seq_to_2d(curr_Q, size_2d) # [1, 128, 31, 54]
        curr_V = silu(curr_QV[1]) # [1674, 1, 512]
        curr_U = self.linear_U(_tgt) # [1674, 1, 512]

        if tgt_id is None:
            tgt_id = 0
            cat_curr_U = torch.cat(
                [silu(curr_U), torch.ones_like(curr_U)], dim=-1) # [1674, 1, 1024]
            curr_ID_V = None
        else:
        ...
        cat_global_V = torch.cat([global_V, global_ID_V], dim=-1) # [1674, 1, 1024]
        cat_local_V = torch.cat([local_V, local_ID_V], dim=1) # [1, 1024, 31, 54]

        cat_tgt2, attn_map = self.long_term_attn(curr_Q, global_K, cat_global_V,
                                          cat_curr_U, size_2d) # cat_tgt2 [1674, 1, 512]
        cat_tgt3, _ = self.short_term_attn(local_Q, local_K, cat_local_V,
                                           cat_curr_U, size_2d) # cat_tgt3 [1674, 1, 512]

        tgt2, tgt_id2 = torch.split(cat_tgt2, self.d_model, dim=-1) # [1674, 1, 256] [1674, 1, 256]
        tgt3, tgt_id3 = torch.split(cat_tgt3, self.d_model, dim=-1) # [1674, 1, 256] [1674, 1, 256]
        ...
        q = k = v = u = torch.cat([_tgt, _tgt_id], dim=-1) # [1674, 1, 512]
        ...
        return tgt, tgt_id, [[curr_K, curr_V, None, curr_ID_V],
                             ...
                             [local_K, local_V, None, local_ID_V]], attn_map
